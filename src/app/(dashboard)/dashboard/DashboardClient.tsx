...
export default function DashboardClient({ initialFleetData }: { initialFleetData: any[] }) {
    ...
    // KPI Calculations
    const totalVehicles = initialFleetData.length;
    const activeFleetCount = initialFleetData.filter(v => v.status === "On Trip").length;
    const maintenanceCount = initialFleetData.filter(v => v.status === "In Shop").length;
    ...
    const filteredData = useMemo(() => {
        return initialFleetData.filter((item) => {
            const q = searchQuery.toLowerCase();
            const matchesSearch = item.vehicle.toLowerCase().includes(q) ||
                item.driver.toLowerCase().includes(q) ||
                item.id.includes(q);
            const matchesType = filterType === "All Types" || item.type.includes(filterType);
    ...
    }, [searchQuery, filterType, filterStatus, filterRegion, initialFleetData]);
